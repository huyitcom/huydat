...
const FEMALE_TEMPLATES = [
  ...
  },
  {
    ...
    prompt: "Create a Vertical shot, characterized by stark cinematic lighting and intense contrast. Captured with a slightly low, upward-facing angle that dramatizes the subject's jawline and neck, the composition evokes quiet dominance and sculptural elegance. The background is a deep, saturated crimson red, creating a bold visual clash with the model's luminous skin and dark wardrobe. Lighting is tigaily directional, casting warm golden highlights on one side of the face while plunging the other into velvety shadow, emphasizing bone structure with almost architectural precision. wearing smart black suit and black shirt. Posing: standing with crossed arms. Shooting haft body. IMPORTANT: Keep the original body posture and posing. The face structure and features must be preserved 100% identically to the original photo."
  },
  {
    label: "Hồ sơ cao cấp",
...
export const ProfilePhotoTab: React.FC = () => {
  ...
  // New state to manage prompt visibility and active template label
    ...
    try {
      ...
      for (const p of promptsToRun) {
        try {
            ...
            // Add delay if there are more items to run to respect rate limits
      ...
      if (currentResults.every(r => !r.imageUrl)) {
          throw new Error("Tạo ảnh thất bại. Vui lòng thử lại sau.");
      }
      ...
      <div className="lg:col-span-4 bg-slate-800/50 p-6 rounded-2xl shadow-lg border border-slate-700 h-fit">
        <div className="space-y-6">
          ...
          <div className="space-y-4">
             {showPromptInput ? (
                <PromptInput
                  label="2. Mô tả chỉnh sửa của bạn"
                  placeholder="VD: 'Ảnh chân dung chuyên nghiệp, nền văn phòng mờ' hoặc 'Ảnh chân dung đen trắng nghệ thuật'"
                  rows={4}
                  value={prompt}
                ...
                <div className="space-y-2">
                    ...
                    <div className="bg-slate-700/50 p-3 rounded-lg border border-slate-600 flex items-center justify-between animate-fade-in transition-all">
                        <div className="flex items-center space-x-2 overflow-hidden mr-2">
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-cyan-500 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
                                <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                            </svg>
                            <span className="text-sm text-slate-300 truncate">
                                Đang dùng mẫu: <span className="font-semibold text-white">{selectedTemplateLabel}</span>
                            </span>
                        </div>
                        <button
                            onClick={() => {
                            ...
                            title="Đặt lại về mô tả tùy chỉnh"
                        >
              ...
              <div className="grid grid-cols-3 gap-2 max-h-[400px] overflow-y-auto pr-1 custom-scrollbar">
                  {currentTemplates.map((template, index) => (
                    <button
                      ...
                      aria-label={`Áp dụng mô tả ${template.label}`}
                    >
                      <img 
                        src={template.image} 
                        alt={template.label} 
           ...
           <div className="space-y-2">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between bg-slate-700/30 p-3 rounded-xl border border-slate-600/50">
                <div>
                  <label htmlFor="image-size-profile" className="text-sm font-medium text-slate-200">
                    Độ phân giải
                  </label>
                  <p className="text-xs text-slate-400 mt-0.5">Chọn chất lượng ảnh đầu ra</p>
                </div>
                <select
                  id="image-size-profile"
                  value={imageSize}
                  onChange={(e) => setImageSize(e.target.value)}
                  disabled={isLoading}
                  className="bg-slate-800 border border-slate-600 text-slate-200 text-sm rounded-lg focus:ring-cyan-500 focus:border-cyan-500 block p-2"
                >
              ...
              <PreserveFaceToggle
                ...
                disabled={isLoading}
                label="Giữ khuôn mặt & tư thế"
                description="Cố gắng giữ nguyên khuôn mặt, biểu cảm và tư thế cơ thể ban đầu."
              />
              <PreserveFaceToggle
                ...
                disabled={isLoading}
                label="Giữ trang phục gốc"
                description="Tạo thêm một phiên bản với trang phục gốc."
              />
            </div>
          </div>
          <EditButton 
